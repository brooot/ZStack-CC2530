/**************************************************************************************************
  ...
  Description:    Sink/Coordinator  汇聚/调制节点
...
/*********** 注意: 每个设备不一样,记得修改!  **********/
unsigned char Life_of_AgentNode = 0;  //代理节点生命周期



int16 AgentNode = (int16)65534;  //存放Sink节点的代理节点
...
PKT Decoded_X[40];     //初始化已解码的码字数组
uint8 X_Num = 0;        //记录已经解码的码字个数

PKT Undecoded_Y[120];   //初始化未解码的复杂码字数组
uint8 Y_Num = 0;        //记录尚未解码的码字个数

int16 Building_AgentNode;      //待进行码字交换的邻居

uint8 Waiting_Request = FALSE;   //是否在等待对方返回交换的数据包回应
//uint8 Exchange_Lock = FALSE;     //码字槽操作锁
...
void Show_Bit(uint16 X, uint8 N); //串口打印一个8位比特二进制数字
void Show_Num(uint16 X);  //串口打印8bit数字
uint16 Power_of_2(uint8 X); //2的多少倍
uint8 Dist_X(uint16 s); //在X集合中但是不在码字中的设备ID个数
void Decode(int8 data, uint16 info);  //解码函数
/*********************************************************************
...
void Show_Bit( uint16 X, uint8 N )
{
  uchar Bit[2] = {'0','1'};
  uint8 index;
...
uint8 Dist_X(uint16 s)
{
  uint16 Temp_X = 0; //所有X中码字的联合表示数字
  for(unsigned short i=0;i<X_Num;++i)
  {
    Temp_X |= Decoded_X[i].Degree_ID;  //将所有的一度码字包含在Comb_X中
  }
  Temp_X = (~Temp_X) & s;  //得到s比X多的码字信息
  if (Temp_X==0)
    return 0;
  else
  {
...
//Dist_X()为1时,解码并存入X中
void Decode(int8 data, uint16 info)
{
  /*距离为1就递归解码*/
  if( Dist_X(info) == 1 )
  {
    ...
    for(short i=0;i<X_Num;++i)
      if(Decoded_X[i].Degree_ID & info)
      {
        De_Data ^= Decoded_X[i].data; //依次异或解码
        De_info &=(~Decoded_X[i].Degree_ID); //去除相应的码字信息
      }
    Decoded_X[X_Num].data = De_Data;
    Decoded_X[X_Num].Degree_ID = De_info;
    HalUARTWrite(0,"解码出:",7);
    Show_Bit(De_Data,8);
    HalUARTWrite(0," ",1);
    Show_Bit(De_info,16);
    HalUARTWrite(0,"\n",1);
    ...
    /*递归解码*/
    for(short j=0;j<Y_Num;++j)
    {
      if( Dist_X(Undecoded_Y[j].Degree_ID) == 1 )  //如果Y中的某项距离为1, 则再次解码
      {
        /*暂存码字与其信息*/
        int8 temp_data = Undecoded_Y[j].data;
        uint16 temp_info = Undecoded_Y[j].Degree_ID;
        /*将其从Y中移除*/
        Undecoded_Y[j] = Undecoded_Y[--Y_Num]; //同时Y的个数减一
        /*解码*/
        Decode(temp_data, temp_info); 
...
//  uint16 Source_DATA_info = Power_of_2(Device_ID);
  //给码字槽赋初值(除了sink节点)
//  for(short i=0;i<6;++i)
//  {
//    DATA_SLOT[i].data = SOURCE_DATA;    //给码字槽赋初值
//    DATA_SLOT[i].Degree_ID = Source_DATA_info; //给码字赋节点信息
//    
//  }
...
//    Show_Bit(SOURCE_DATA,8);
    HalUARTWrite(0," ",1);
//    Show_Bit(Power_of_2(Device_ID),16);
    HalUARTWrite(0,"\n",1);
//
    for(int8 p=1;p<6;++p)
    {
      ...
      HalUARTWrite(0," info:",6);
      Show_Bit(DATA_SLOT[p].Degree_ID, 4);
      HalUARTWrite(0,"\n",1);
    }
    
    //显示邻居个数
    uint8 nei_N = 0;
    for(uchar num_nei=0;num_nei<3;++num_nei)
    {
      if(Nei_Table[num_nei] != (int16)65534)
        nei_N++;
    }
    HalUARTWrite(0,"nei_num:",8);
    Show_Num(nei_N);
    HalUARTWrite(0,"\n",1);
...
//      HalUARTWrite(0,"\n",1);
      ...
      for(int8 p=0;p<1;++p)
      {
        ...
        Show_Bit(256, 16);
        HalUARTWrite(0," ",1);
        Show_Num(DATA_SLOT[p].Degree_ID);
        HalUARTWrite(0,"\n",1);
      }
    }
...
//  unsigned char ADDR[4]; //数字地址转换后的字符地址
  ...
  uint16 Peer_info; //对方码字信息

//  int8 degree_low; //码字信息低位
//  int8 degree_high;//码字信息高位
//  uint16 Source_info = Power_of_2(Device_ID);
  ...
  case SAMPLEAPP_POINT_TO_POINT_CLUSTERID:
    ...
    /*表示指定的代理节点已经建立*/
    else if( (neighbor_addr_10 == Building_AgentNode) && (pkt->cmd.Data[0] == '%') && (pkt->cmd.Data[1] == '%') && (pkt->cmd.Data[2] == '%') && (pkt->cmd.Data[3] == '%') )
    {
      Life_of_AgentNode = 3;  //设置代理节点的存活周期
      AgentNode = neighbor_addr_10; //记录代理节点的地址
      HalUARTWrite(0,"代理节点已建立\n",15);
      Waiting_Request = FALSE; //请求得到相应
    }
    
    /*收到代理节点发来的数据信号*/
    else if((neighbor_addr_10 == AgentNode) && (pkt->cmd.Data[0] == '+'))
    {
      Waiting_Request = FALSE; //请求得到相应
      Life_of_AgentNode = 3;  //刷新代理节点的存活周期
      Peer_data = pkt->cmd.Data[1]; //代理节点发来的数据
      Peer_info = pkt->cmd.Data[3];
      Peer_info |= pkt->cmd.Data[2]*256; //码字信息
      
      
      Decode(Peer_data, Peer_info);
    }
    
    HalUARTWrite(0, "收到代理节点信息:", 17);  //给串口发送提示
    HalUARTWrite(0, &pkt->cmd.Data[0], 1); // 打印收到数据
    
    Show_Bit(pkt->cmd.Data[1],8);
    HalUARTWrite(0," ",1);
    Show_Bit(pkt->cmd.Data[2],8);    //打印高8位码字编码信息
    HalUARTWrite(0," ",1);
    Show_Bit(pkt->cmd.Data[3],8);
    HalUARTWrite(0, "\n", 1); // 回车换行
    
    
    HalLedBlink( HAL_LED_2, 2, 60, (flashTime / 4) ); //二号灯闪烁,两次,亮度60%,时间间隔0.125s
    break;
...
void SampleApp_SendPointToPointMessage( void )
{
  ...
  /*当代理节点生命为0的时候*/
  if(Life_of_AgentNode == (int16)0)
  {
    AgentNode = (int16)65534; //重置代理节点地址
    
    /*选择一个代理节点,向随机邻居发送建立代理关系请求,并要求回应.*/
    int8 nei_index = random(NEI_NUM);  //挑选随机邻居节点
    Building_AgentNode = Nei_Table[nei_index];  //即将发送建立代理关系的邻居
    
    uint8 _DATA[] = {'@', '@', '@', '@'};  //发送的代理节点指派命令
    
    /*给选出的邻居发送数据*/
    if((Nei_Table[nei_index] != (int16)65534) && (0<=nei_index) && (nei_index<NEI_NUM))
    {
      Point_To_Point_DstAddr.addr.shortAddr = Building_AgentNode;
      if ( AF_DataRequest( &Point_To_Point_DstAddr, //点对点通信方式目标地址
                          &SampleApp_epDesc,
                          SAMPLEAPP_POINT_TO_POINT_CLUSTERID, //点对点簇ID
                          4,
                          ...
                          AF_DEFAULT_RADIUS ) == afStatus_SUCCESS )
      {
        HalUARTWrite(0,"发送代理请求\n",13);
        Waiting_Request = TRUE; //请求回复状态中
        HalLedBlink( HAL_LED_1, 3, 60, (flashTime / 3) ); //led1闪
      }
      else
      {
        HalUARTWrite(0,"代理请求发送失败.",strlen("交换数据包请求发送失败."));
      }
    }
    else
    {
      HalUARTWrite(0,"指派代理失败\n",13);
    }
  }
  ...
  {
    /*向代理节点发送存活确认,如果得到回复就把代理生命设置为最大值*/
    
    uint8 Check_DATA[] = {'?', '?', '?', '?'};  //发送的代理节点存活确认命令
    
    /*给选出的邻居发送数据*/
    Point_To_Point_DstAddr.addr.shortAddr = AgentNode;
    if ( AF_DataRequest( &Point_To_Point_DstAddr, //点对点通信方式目标地址
                        &SampleApp_epDesc,
                        SAMPLEAPP_POINT_TO_POINT_CLUSTERID, //点对点簇ID
                        4,
                        ...
                        AF_DEFAULT_RADIUS ) == afStatus_SUCCESS )
    {
      HalUARTWrite(0,"发送代理存活确认\n",13);
      /*如果发送请求的时候仍然没有收到对方的消息,代理节点的生命-1*/
      if(Waiting_Request == TRUE)
      {
        --Life_of_AgentNode;
      }
      HalLedBlink( HAL_LED_3, 1, 60, (flashTime / 2) ); //led1闪
    }
    
    else
    {
      HalUARTWrite(0,"存活确认发送失败\n",strlen("存活确认发送失败\n"));
    }
  }
}
