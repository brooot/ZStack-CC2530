...
//uint8 SOURCE_DATA = 1<<0;    //采集的数据 1
//uint8 Device_ID  = 0;    //设备ID

//uint8 SOURCE_DATA = 1<<1;    //采集的数据 2l
//uint8 Device_ID  = 1;    //设备ID

//uint8 SOURCE_DATA = 1<<2;    //采集的数据 3
//uint8 Device_ID  = 2;    //设备ID

uint8 SOURCE_DATA = 1<<3;      //采集的数据 4
uint8 Device_ID  = 3;      //设备ID
...
PKT DATA_SLOT[5+1];             //码字结构体数组,0位保存自身数据,用1-5位来进行码字交换
...
unsigned short Is_AgentNode = FALSE; //是否是代理节点
/*<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
* end
*/
...
//  unsigned char ADDR[4]; //数字地址转换后的字符地址
  ...
  case SAMPLEAPP_POINT_TO_POINT_CLUSTERID:
    ...
    /*收到指定为代理节点消息*/
    else if ((pkt->cmd.Data[0] == '@') && (pkt->cmd.Data[1] == '@') && (pkt->cmd.Data[2] == '@') && (pkt->cmd.Data[3] == '@'))
    {
      Is_AgentNode = TRUE; //设置为是代理节点
      
      /*回复Sink节点已经设置自身为代理节点*/
      uint8 set_Agent_succeed[] = {'%', '%', '%', '%'};  //发送给sink节点的数据,用 '+' 标识
      
      /*给Sink节点发送数据*/
      Point_To_Point_DstAddr.addr.shortAddr = (int16)0;
      if ( AF_DataRequest( &Point_To_Point_DstAddr, //点对点通信方式目标地址
                          ...
                          AF_DEFAULT_RADIUS ) == afStatus_SUCCESS )
      {
        HalLedBlink( HAL_LED_3, 4, 60, (flashTime / 4) ); //led3闪
      }
      else
      {
        HalUARTWrite(0,"向Sink节点发送码字失败\n",strlen("向Sink节点发送码字失败\n"));
      }
    }
    /*收到交换码字信息*/
    else if((pkt->cmd.Data[0] == '~') && (pkt->cmd.Data[1] == '~') && (pkt->cmd.Data[2] == '~') && (pkt->cmd.Data[3] == '~'))
      ...
      if(Is_AgentNode)
          {
            ...
            uint8 send_to_sink_index =  random(5)+1; //随机产生一个发送给sink节点的码字的序列号
            uint8 to_sink_data = DATA_SLOT[send_to_sink_index].data; //即将发送给Sink节点的码字
            uint8 to_sink_degree_high = DATA_SLOT[send_to_sink_index].Degree_ID >>8; //高位码字度信息
            uint8 to_sink_degree_low = (uint8)DATA_SLOT[send_to_sink_index].Degree_ID;  //低位码字度信息
            uint8 to_sink_DATA[] = {'+', to_sink_data, to_sink_degree_high, to_sink_degree_low};  //发送给sink节点的数据,用 '+' 标识
            ...
            if ( AF_DataRequest( &Point_To_Point_DstAddr, //点对点通信方式目标地址
                                ...
                                AF_DEFAULT_RADIUS ) == afStatus_SUCCESS )
            {
              HalLedBlink( HAL_LED_3, 2, 60, (flashTime / 2) ); //led3闪
            }
            else
            {
              HalUARTWrite(0,"向Sink节点发送码字失败\n",strlen("向Sink节点发送码字失败\n"));
    ...
    {
      HalUARTWrite(0, "received  ", 10);  //给串口发送提示
      ...
      if(Waiting_Request && (neighbor_addr_10 == Exchanging_Neighbor)) 
      {
        ...
        if(!Exchange_Lock && (pkt->cmd.Data[0] == '$'))
        {
          ...
          Peer_info |= pkt->cmd.Data[2]*256; //对方码字信息
          //查看发来的数据包的度
          uint8 Peer_degree = 0;
          uint8 Self_included = FALSE;
          for(short k=0;k<16;++k)
          {
            if(Power_of_2(k) & Peer_info)
            {
              ++Peer_degree;
              if(k == Device_ID)
              {
          ...
          if(Peer_degree>=MAX_Degree)
          {
            if(Self_included)
              HalUARTWrite(0,"且",2);
            HalUARTWrite(0,"已达最大度,",11);
          }
          else
          {
            HalUARTWrite(0,",",1);
          ...
          {
            HalUARTWrite(0,"encoding...\n^^",14);
            //          Show_Bit(Peer_data,8);          //对方数据
            //          HalUARTWrite(0,"\n^^",3);
            //          Show_Bit(DATA_SLOT[0].data,8);  //自身数据
            //          
            //          HalUARTWrite(0,"\n->",3);
            //          Peer_data ^= DATA_SLOT[0].data; //异或编码
            //          
            //          Show_Bit(Peer_data, 8);         //编码后数据
            
            HalUARTWrite(0,"\n||",3);
            Show_Bit(Peer_info,8);
            HalUARTWrite(0," ",1);
            Show_Num(Peer_info);          //对方码字信息
            
            HalUARTWrite(0,"\n||",3);
            Show_Bit(Source_info,8);  //**
            HalUARTWrite(0," ",1);
            Show_Num(Source_info);         //自身码字信息
            
            HalUARTWrite(0,"\n->",3);      
            Peer_info |= Source_info; //码字信息中添加自身的设备ID
            Show_Bit(Peer_info,8);   //**
            HalUARTWrite(0," ",1);
            Show_Num(Peer_info);          //融合后码字信息
            HalUARTWrite(0,"\n",1);
          }
          else
          {
        ...
        /*如果收到对方发来的交换数据包请求*/
        else if(!Waiting_Request && !Exchange_Lock && (pkt->cmd.Data[0] == '*'))
        {
          Exchange_Lock = TRUE; //开启码字槽的锁
          /*挑选自身随机的数据包发送给对方*/
          ...
          degree_high = (uint8)(Self_info>>8);  //此处不加括号会出错
          ...
          if(AF_DataRequest( &Point_To_Point_DstAddr, //点对点通信方式目标地址
                            ...
                            AF_DEFAULT_RADIUS )== afStatus_SUCCESS )
          {
            HalUARTWrite(0,"send back $",11);
            Show_Bit(Self_data,8);
            HalUARTWrite(0," ",1);
            //        Show_Bit(degree_high,8);
            //        HalUARTWrite(0," ",1);
            Show_Bit(degree_low,8);
            HalUARTWrite(0,"\n",1);
          }
          else
          {
            HalUARTWrite(0,"failed to send back\n",20);
          }
        ...
        HalLedBlink( HAL_LED_2, 2, 60, (flashTime / 6) ); //二号灯闪烁,两次,亮度60%,时间间隔0.125s
      }
    }
    break;
...
void SampleApp_SendPointToPointMessage( void )
{
  ...
  if((Nei_Table[nei_index] != (int16)65534) && (0<=nei_index) && (nei_index<NEI_NUM))
  {
    ...
    HalUARTWrite(0,"send      *", 11);
    
    Show_Bit(Exchanging_Data, 8);
    HalUARTWrite(0," ",1);
//    Show_Bit(degree_high,8);
//    HalUARTWrite(0," ",1);
    Show_Bit(degree_low,8);
    
    HalUARTWrite(0," to addr ", 9);
